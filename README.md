...
| Job `lifecast_engine_run` — **the mock engine, orchestrated** | Consumes the exported MPF + approved basis + curve, values the term book per model point (base + ±100bp sensitivities), synthesises the two legacy-fed lines, dumps estate results CSVs — behaving exactly like the real engine from the outside |
| Job `lifecast_quarter_close` | The whole close as one workflow: overnight run → engine run → results run |
| Pipeline `lifecast_results_pipeline` + job `lifecast_results_run` | Results dumps → governed layer (`gld_results_by_product`, `gld_bel_movement`, per-MP `slv_engine_mp_results`; latest engine run wins on re-runs); then CFO export, dashboard publish, Genie space, **BEL analytics** (`gld_bel_sensitivity` rate-risk map, `gld_bel_concentration`, `gld_movement_by_cohort`) |
