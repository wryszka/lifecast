...
| Job `lifecast_engine_run` — **the mock engine, orchestrated** | Consumes the exported MPF + approved basis + curve, values the term book per model point (base + ±100bp sensitivities), synthesises the two legacy-fed lines, dumps estate results CSVs **+ its run log** — behaving exactly like the real engine from the outside |
| Job `lifecast_quarter_close` | The whole close as one workflow: overnight run → engine run → results run |
| Pipeline `lifecast_results_pipeline` + job `lifecast_results_run` | Results dumps → governed layer (`gld_results_by_product`, `gld_bel_movement`, per-MP `slv_engine_mp_results`, run logs `slv_engine_run_log`; latest engine run wins on re-runs); then **BEL analytics** (`gld_bel_sensitivity` rate-risk map, `gld_bel_concentration`, `gld_movement_by_cohort`), **run audit** (`gld_run_audit`), CFO export (board + rate-risk + audit sheets), dashboard publish, Genie space |
| `gld_run_audit` (built by `06_run_audit`) | **The audit trail** — one row per engine run: input file + gate verdict, basis version + approver + when, curve date, operator, timings, minutes-to-queryable. Plus Delta history + `VERSION AS OF` for "reproduce that number" |
| `LifeCast — BEL Movement` dashboard · `LifeCast — Results` Genie space | The reporting destinations: page 1 counters/trend/movement, page 2 **risk & audit** (rate-risk map, concentration, cohort movers, run audit); Genie answers "BEL movement vs last quarter by product line" and "who ran the latest engine run, on which basis?" |
